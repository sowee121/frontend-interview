...
3. **收尾**：一句边界或业务场景（可选单独一段）。
4. **禁止出题人 / 备考视角文字**：答案里只放知识。以下黑名单一律不进正文与 `questionNote`：
   - 元信息前缀：`考察要点：`、`对应职责：`、`对应经历：`
   - 备考口吻：`面试`、`追问`、`答题`、`背诵`、`得分`、`加分点`
   - 答题指令：`要说清`、`须说明`、`需理解`、`要提到`、`要能列`、`不必背`、`少背`、`硬背`、`口述`
   - 备注里的弱指令：`即可`、`无需`、`不必`（如「掌握一条规则即可」）；正文中「重试即可」这类技术表述不受限
   - 时效提示（如「Vue 2 已于某日 EOL」）：不写日期，只保留选型结论
   - 简历映射（`对应职责：…`）：`questionNote` 整条删除，而不是留空串
...
- `lead` 只写该章的**知识主线**（本章讲什么、按什么顺序），与 `description` 分工：`description` 列范围，`lead` 讲主线。
- `lead` 不写复习指引：`略读`、`口述`、`准备`、`深挖点`、`为主`、`转到`、`见「某章」`、`即可` 等；`lint-qa-copy` 的 `editorial-hint` 会拦截。
